...
class Add_kandidat_public extends CI_Controller {
...
//       $date = sha1(date('dmy'));
//       $gen = md5($date);
        $t['token'] = sha1(date('dmy'));
        $t['key'] = md5($t['token']);
        if ($token == $t['token'] && $key == $t['key']) {
            ...
            //$a['kandidat']['datatable'] = $this->load->view('public_kandidat/vadd_kandidat', $t, true);
            $a['template']['header'] = $this->load->view('template/vheader', NULL, true);

            $this->form_validation->set_rules('nama_lengkap', 'Nama Lengkap', 'trim|required');
            //$this->form_validation->set_rules('no_ktp', 'Nomor KTP', 'trim|required|calback_ktp_handle');
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                redirect(base_url() . "thanks-pages?token=" . $t['token'] . "&key=" . $t['key'] . "&secret_key=" . sha1($t['key']));
            }
            ...
            if ($prov_id != "") {
                ...
                $a['list'] = $this->mkandidat->get_kandidat_list();
                $a['kandidat']['datatable'] = $this->load->view('kandidat/vadd_kandidat_public', $t, true);
                $this->load->view('pages/vkandidat_public', $a, FALSE);
            } else {
                $t['dropdownprov'] = $this->mkandidat->cms_list_prov();
            ...
            redirect('http://www.valdoinc.com');
        } else {
            redirect('http://www.valdoinc.com');
        }
    }
