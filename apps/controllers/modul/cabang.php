...
class Cabang extends CI_Controller {
	...
	public function __construct()
	{
            ...
            $this->load->model('modul/mcabang','mcabang');
	...
	public function index()
	{
                ...
                $a['admin']['datatable']= $this->load->view('modul/cabang',NULL,true);
        ...
        public function ajax_list()
	{
		...
		foreach ($list as $admin){
			...
			$row[] = '<a data-toggle="tooltip" class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$admin->id_cabang."'".')"><i class="glyphicon glyphicon-pencil"></i></a>
				  <a data-toggle="tooltip" class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$admin->id_cabang."'".')"><i class="glyphicon glyphicon-trash"></i></a>';
			$data[] = $row;}
		$output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->mcabang->count_all(),
                "recordsFiltered" => $this->mcabang->count_filtered(),
                "data" => $data,);
		echo json_encode($output);
	}
	public function ajax_edit($id)
	{
            $data = $this->mcabang->get_by_id($id);
	...
	public function ajax_update()
	{
            ...
            $this->mcabang->update(array('id_cabang' => $this->input->post('id_cabang')), $data);
            echo json_encode(array("status" => TRUE));
	}
        
	public function ajax_delete($id)
	{
            $this->mcabang->delete_by_id($id);
            echo json_encode(array("status" => TRUE));
	}
