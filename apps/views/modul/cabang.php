...
<script type="text/javascript">
            ...
            $(document).ready(function () {
                table = $('#table').DataTable({
                    ...
                    "order": [],
                    "ajax": {
                        "url": "<?php echo site_url('modul/cabang/ajax_list') ?>",
                        "type": "POST"
                    },
                    "columnDefs": [
                        {
            ...
            function edit_person(id_cabang)
            {
                save_method = 'update';
                $('#form')[0].reset();
                $('.form-group').removeClass('has-error');
                $('.help-block').empty();
                $.ajax({
                    url: "<?php echo site_url('modul/cabang/ajax_edit') ?>/" + id_cabang,
                    type: "GET",
                    dataType: "JSON",
            ...
            function save()
            {
                ...
                if (save_method == 'add') {
                    url = "<?php echo site_url('modul/cabang/ajax_add') ?>";
                } else {
                    url = "<?php echo site_url('modul/cabang/ajax_update') ?>";
                }
            ...
            function delete_person(id_cabang)
            {
                if (confirm('Are you sure delete this data?'))
                {
                    // ajax delete data to database
                    $.ajax({
                        url: "<?php echo site_url('modul/cabang/ajax_delete') ?>/" + id_cabang,
                        type: "POST",
                        dataType: "JSON",
                        success: function (data)
                        {
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="nama_cabang" placeholder="Nama Cabang" class="form-control" type="text">
                            </div>
                        </div>
                    </div>
                </form>
