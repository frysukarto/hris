...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('modul/hrd/ajax_list') ?>",
                "type": "POST"
            },
            "columnDefs": [
                {
    ...
    function edit_person(id_kar)
    {
        ...
        $('.help-block').empty();
        $.ajax({
            url: "<?php echo site_url('modul/hrd/ajax_edit/') ?>/" + id_kar,
            type: "GET",
            dataType: "JSON",
            success: function (data)
            {
                $('[name="id_kar"]').val(data.id_kar);
                $('#modal_form').modal('show');
    ...
    function save()
    {
        ...
        var url;
        if (save_method == 'add') {
            url = "<?php echo site_url('modul/hrd/ajax_add') ?>";
        } else {
            url = "<?php echo site_url('modul/hrd/ajax_update') ?>";
        }
    ...
    function delete_person(id_kar)
    {
        if (confirm('Are you sure delete this data?'))
        {
            // ajax delete data to database
            $.ajax({
                url: "<?php echo site_url('modul/hrd/ajax_delete') ?>/" + id_kar,
                type: "POST",
                dataType: "JSON",
                success: function (data)
                {
